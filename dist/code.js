...
var TIME_SLOTS_COLUMN$1 = 5;
var TEAM_SIZE$1 = 5;

function getTimeSlots$1() {
...
var Logger$2 = console; // For simplicity in a Node.js environment
...
function manageTimeSlots() {
  var ui = SpreadsheetApp.getUi();
  var GAME_DAY = getGameDay$1();
  var TIME_SLOTS = getTimeSlots$1();
...
function setAutomaticTimeSlots() {
  ...
  var timeSlotsRange = sheet.getRange(2, TIME_SLOTS_COLUMN$1, sheet.getLastRow() - 1, 1);
...
function sortPlayersIntoBalancedTeams() {
  Logger$2.log("sortPlayersIntoBalancedTeams function started");

  // Update Time and Day variables
  var TIME_SLOTS = getTimeSlots$1(); // Refresh TIME_SLOTS at the start of the function
  ...
  try {
    ...
    Logger$2.log("Sheets retrieved successfully");
    var allPlayers = getPlayersData(playersSheet);
    Logger$2.log("All players data retrieved: " + JSON.stringify(allPlayers));
    ...
    Logger$2.log("Teams and substitutes created: " + JSON.stringify(teamsAndSubs));
    writeTeamsToSheet(teamsSheet, teamsAndSubs, TIME_SLOTS);
    Logger$2.log("Teams written to sheet");
  } catch (e) {
    Logger$2.log("Error: ".concat(e.message));
    throw e;
  }
  Logger$2.log("sortPlayersIntoBalancedTeams function completed");
}
...
function createOptimalTeams(players, TIME_SLOTS) {
  ...
  if (!TIME_SLOTS || TIME_SLOTS.length === 0) {
    Logger$2.log("Error: TIME_SLOTS is undefined or empty");
    return result;
  }
  ...
  TIME_SLOTS.forEach(function (timeSlot, slotIndex) {
    ...
    assignedPlayers = new Set([].concat(_toConsumableArray$1(assignedPlayers), _toConsumableArray$1(slotResult.assignedPlayers)));
    Logger$2.log("Created ".concat(slotResult.teams.length, " teams for time slot: ").concat(timeSlot));
    Logger$2.log("Substitutes for time slot ".concat(timeSlot, ": ").concat(slotResult.substitutes.length));
  });
  return result;
}
function createOptimalTeamsForTimeSlot(players, timeSlot, assignedPlayers) {
  var numPlayers = players.length;
  var maxTeams = Math.floor(numPlayers / TEAM_SIZE$1);
  ...
  var teamPlayers = players.slice(0, adjustedNumTeams * TEAM_SIZE$1);
  ...
  var substitutes = players.slice(adjustedNumTeams * TEAM_SIZE$1);
  ...
  Logger$2.log("Team spread for ".concat(timeSlot, ": ").concat(teamSpread.toFixed(2)));
  return {
    teams: teams,
...
function generateDiscordPings() {
  ...
  if (!teamsSheet) {
    Logger$2.log("Error: Teams sheet not found.");
  ...
  Logger$2.log("Total rows in Teams sheet: ".concat(data.length));
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    Logger$2.log("Processing row ".concat(i + 1, ": \"").concat(firstCell, "\""));
    ...
    if (row.length === 6 && (firstCell.includes("CEST") || firstCell.includes("WEST"))) {
      ...
      Logger$2.log("Detected Time Slot: \"".concat(currentTimeSlot, "\""));
      continue;
    }
    ...
    if (firstCell.startsWith("Team")) {
      ...
      currentSection = "team";
      Logger$2.log("Detected Team: \"".concat(currentTeam.name, "\" under Time Slot: \"").concat(currentTimeSlot, "\""));
      continue;
    }
    ...
    if (firstCell === "Discord" && currentSection === "team") {
      ...
      Logger$2.log("Detected Players section under Team: \"".concat(currentTeam.name, "\""));
      continue;
    }
    ...
    if (firstCell === "Substitutes") {
      ...
      Logger$2.log("Detected Substitutes section under Time Slot: \"".concat(currentTimeSlot, "\""));
      continue;
    }
    ...
    if (firstCell === "Discord" && currentSection === "substitutes") {
      ...
      Logger$2.log("Detected Players section under Substitutes for Time Slot: \"".concat(currentTimeSlot, "\""));
      continue;
    }
    ...
    if (currentSection === "players" && firstCell !== "") {
      ...
      Logger$2.log("Added Player to Team \"".concat(currentTeam.name, "\": \"@").concat(player.discordUsername, "\" (Lobby Host: ").concat(player.lobbyHost, ")"));
      continue;
    }
    ...
    if (currentSection === "substitutesPlayers" && firstCell !== "") {
      ...
      Logger$2.log("Added Substitute to Time Slot \"".concat(currentTimeSlot, "\": \"@").concat(substitute.discordUsername, "\" (Lobby Host: ").concat(substitute.lobbyHost, ")"));
      continue;
    }
    ...
    if (firstCell === "") {
      currentSection = null;
      Logger$2.log("Encountered empty row. Resetting current section.");
      continue;
    }
  }
  Logger$2.log("Total Teams Parsed: ".concat(teams.length));
  Logger$2.log("Total Substitutes Parsed: ".concat(JSON.stringify(substitutes)));
  ...
  Logger$2.log("Game Day: \"".concat(gameDay, "\", Date: \"").concat(formattedDate, "\""));
  ...
  timeSlots.forEach(function (timeSlot) {
    ...
    if (timeSlotTeams.length > 0 || timeSlotSubstitutes.length > 0) {
      ...
      Logger$2.log("Added Time Slot Header: \"".concat(timeSlot, " Timeslot\""));

      // Group teams into pairs (e.g., Team 1 & Team 2)
      for (var _i = 0; _i < timeSlotTeams.length; _i += 2) {
        var pair = timeSlotTeams.slice(_i, _i + 2);
        Logger$2.log("Processing Team Pair: \"".concat(pair.map(function (t) {
          return t.name;
        }).join(" & "), "\""));
        ...
        if (lobbyHosts.length > 0) {
          ...
          Logger$2.log("Selected Lobby Host: \"@".concat(selectedHost.discordUsername, "\" for Team Pair: \"").concat(pair.map(function (t) {
            return t.name;
          }).join(" & "), "\""));
        } else {
          Logger$2.log("No Lobby Host found for Team Pair: \"".concat(pair.map(function (t) {
            return t.name;
          }).join(" & "), "\""));
        }
        ...
        if (selectedHost) {
          ...
          Logger$2.log("Added Lobby Host Section: \"@".concat(selectedHost.discordUsername, "\""));
        }

        // Add each team in the pair
        pair.forEach(function (team) {
          contentArray.push("### ".concat(team.name));
          Logger$2.log("Added Team Header: \"".concat(team.name, "\""));
          team.players.forEach(function (player) {
            contentArray.push("@".concat(player.discordUsername));
            Logger$2.log("Added Player Mention: \"@".concat(player.discordUsername, "\""));
          });
          contentArray.push(''); // Blank line after each team
          Logger$2.log("Added blank line after Team: \"".concat(team.name, "\""));
        });
      }
      ...
      if (timeSlotSubstitutes.length > 0) {
        ...
        Logger$2.log("Added Substitutes Header for Time Slot: \"".concat(timeSlot, " Timeslot\""));
        timeSlotSubstitutes.forEach(function (sub) {
          contentArray.push("@".concat(sub.discordUsername));
          Logger$2.log("Added Substitute Mention: \"@".concat(sub.discordUsername, "\""));
        });
        contentArray.push(''); // Blank line after substitutes
        Logger$2.log("Added blank line after Substitutes for Time Slot: \"".concat(timeSlot, " Timeslot\""));
      }

      // Add separator
      contentArray.push(''); // Blank line after separator
      Logger$2.log("Added separator for Time Slot: \"".concat(timeSlot, " Timeslot\""));
    }
  });

  // Join all text for potential direct usage (e.g., sending via API)
  var discordPingText = contentArray.join('\n');
  Logger$2.log("Generated Discord Ping Text:\n" + discordPingText);
  ...
  try {
    ...
    Logger$2.log("Successfully wrote Discord pings to the 'Discord Pings' sheet.");
  } catch (error) {
    Logger$2.log("Error writing to Discord Pings sheet: ".concat(error.message));
    throw error;
  }
...
function writeDiscordPingsToSheet(sheet, pings) {
  sheet.clear();
  Logger$2.log("Cleared existing content in Discord Pings sheet.");
  var lines = pings.split("\n");
  var numRows = lines.length;
  var range = sheet.getRange(1, 1, numRows, 1);
  Logger$2.log("Total lines to write: ".concat(numRows));
  ...
  for (var i = 0; i < numRows; i++) {
    ...
    if (content.startsWith("# ")) {
      ...
      .setFontColor("#ffffff");
      Logger$2.log("Formatted Timeslot Header: \"".concat(content, "\""));
      ...
      Logger$2.log("Formatted Lobby Host Header: \"".concat(content, "\""));
    } else if (content.startsWith("### Team")) {
      // Team Headers: Light Blue
      cell.setFontWeight("bold").setFontSize(13).setBackground("#CFE2F3") // Light Blue
      .setFontColor("#000000"); // Black text
      Logger$2.log("Formatted Team Header: \"".concat(content, "\""));
  ...
  Logger$2.log("Auto-resized and set minimum column width.");
}
...
function clearResponses() {
  ...
  if (lastRow > 1) {
    ...
    if (response == ui.Button.YES) {
      ...
      // Log the action
      Logger$1.log("Cleared all responses from the Forms Responses sheet.");
      ...
      Logger$1.log("Clear responses operation cancelled by user.");
    }
  } else {
...
var Logger$1 = console; // For simplicity in a Node.js environment
